...
fn main() {
    ...
    let index_path = args.get(2).map(|s| s.as_str()).unwrap_or("data/index.bin.gz");

    println!("=== Rinha Backend 2026 - Test Suite (load mode) ===\n");

    let norm = dataset::load_normalization(&format!("{}/normalization.json", data_dir))
        .expect("load normalization");
    let mcc_risk = dataset::load_mcc_risk(&format!("{}/mcc_risk.json", data_dir))
        .expect("load mcc_risk");
    ...
    let test_data: TestData =
        serde_json::from_str(&test_json).expect("Failed to parse test data");
    println!(
    ...
    let max_tests = test_data.entries.len().min(54100);
    ...
    for (i, entry) in test_data.entries.iter().enumerate().take(max_tests) {
        ...
        if i < 5 || (approved != entry.expected_approved && i < 200) {
            let status = if approved == entry.expected_approved { "✓" } else { "✗" };
            println!(
                "  {} tx-{}: expected={}, got={}, score={:.3} (exp={:.3}) [{:.3}ms]",
                status, i, entry.expected_approved, approved,
                fraud_score, entry.expected_fraud_score,
    ...
    println!("  Avg score diff: {:.4}", total_score_diff / max_tests as f32);
