...
unsafe fn scan_and_count(probes: &[usize], ds: &Dataset, q_i16: &[i16; 14]) -> u8 {
    ...
    for &ci in probes {
        ...
        for bi in block_start..block_end {
            ...
            // For each dimension, load 8 i16 values, widen to i32, subtract q_i16[d], square,
            // and distribute across 8 per-vector distance accumulators.
            // This is the proven SSE approach — _mm256_madd_epi16 was attempted in v53 but
            // is incompatible with the SOA block layout (adjacent lanes are same-dim, not same-vec).
            for d in 0..14usize {
